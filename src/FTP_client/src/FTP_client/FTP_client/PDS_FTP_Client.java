...
public class PDS_FTP_Client
{
	...
	public static void main(String[] args)
	{
		...
		if ( ftpClient.connectToPDS( ) )
		{
			...
			try
			{
				...
				System.out.println("main: files loaded");
				Vector<String> paths;
				paths = collectPathFiles(directory);
				System.out.println("paths.size: " + paths.size());
				
				for (int i=0; i<paths.size(); i++)
				{
					ftpClient.loadFile( directory, paths.get(i) );
				}
    ...
    static // Function to collect all files from a defined path, ignoring folders
	Vector<String> collectPathFiles( String path ) throws IOException
	{
		FTPFile [] fileList;
		...
		Vector<String> fileNamesList = new Vector<String>();
		...
		for (int i = 0; i<fileList.length; i++) {
			...
			if (fileList[i].isFile()) {
				...
				fileNamesList.add(fileList[i].getName());
		    }
		}			
